...
2. 대표적인 활용 영역
	- 게시물 글번호에 주로 사용
	- 고객 등록시에 1씩 자동증가해서 고객 관리

3. 시퀀스의 기능
	- 일정 숫자만큼 자동 생성되는 특성
	- nextval : insert문장에서 사용 / 1씩 증가하여 insert
	- currval : 현 시점에서의 sequence 값을 알고자 할 때 확인용 속성
		select 시퀀스이름.currval fom dual;
*/	
...
-- 해당 sequence의 현재 값 검색
select seq_test_no1.currval from dual; --currval
