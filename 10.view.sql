...
--3. ? emp table에서 comm을 제외한 ename, empno, sal 컬럼을 emp01_v 라는 view 생성
drop view emp01_v;
create view emp01_v as select ename, empno from emp;





--4. dept01_v에 crud : dep01_v와 dept01 table 변화 동시 검색 (CRUD 작업 모두 가능)
select * from dept01_v;
insert into dept01_v values(50, '교육부', '홍대');
select * from dept01_v;
select * from dept01; --원본 table에도 반영이 됨
...
-- 빈번히 사용되는 join의 경우 별도의 view 생성해서 검색에 사용하기도 함
...
--7. view 삭제 후 사전 table 재 확인
-- view 삭제시 사전 table에도 자동 삭제
drop view emp01_dept01_v;
select * from user_views;
	

--8. 인라인 뷰
-- emp, dept table에서 검색, 조건은 각 부서별 지역명, 근무하는 사원

select loc, ename 
from (select loc, d.deptno
from emp e, dept d 
where e.deptno = d.deptno;);
